...
class EvaluationEngine:
    ...
    def calculate_hard_match_score(
        ...
        # Fuzzy matches for similar skills (made much more lenient)
        for job_skill in all_job_skills:
            if job_skill not in matched_skills:
                for candidate_skill in candidate_skills:
                    similarity = self._calculate_string_similarity(job_skill, candidate_skill)
                    if similarity >= 30:  # Reduced from 50% to 30% for very lenient matching
                        matched_skills.append(job_skill)
                        break
        ...
        # Score calculation: more balanced weights and bonus scoring for very lenient evaluation
        if len(required_skills) > 0:
            required_score = (required_matched / len(required_skills)) * 0.5  # Reduced from 0.6 to 0.5
        else:
            required_score = 0.5  # Give bonus if no required skills listed
        
        if len(preferred_skills) > 0:
            preferred_score = (preferred_matched / len(preferred_skills)) * 0.5  # Increased from 0.4 to 0.5
        else:
            preferred_score = 0.5  # Full preferred score if no preferred skills listed
        
        # Add bonus for any skill matches (very lenient approach)
        skill_bonus = min(matched_count * 0.1, 0.3)  # Up to 30% bonus for having any matching skills
        
        hard_match_score = (required_score + preferred_score + skill_bonus) * 100
    ...
    def _calculate_simple_text_similarity(
        ...
        """Simple text-based similarity without ML dependencies (made more generous)"""
        try:
            ...
            if not job_text or not resume_text:
                return 20.0  # Give some base score instead of 0
            
            # Simple word overlap calculation
            job_words = set(job_text.split())
            resume_words = set(resume_text.split())
            
            # Calculate Jaccard similarity with bonus
            intersection = len(job_words & resume_words)
            union = len(job_words | resume_words)
            
            if union == 0:
                return 25.0  # Base score for having any text
                
            jaccard_similarity = intersection / union
            
            # Apply generous multiplier and add base bonus
            generous_score = (jaccard_similarity * 150) + 20  # More generous scoring
            return min(generous_score, 100.0)
        
        except Exception as e:
            print(f"Error in semantic matching: {e}")
            return 25.0  # Return reasonable base score instead of 0
    ...
    def determine_suitability(self, relevance_score: float) -> str:
        """Determine suitability level based on score (made very lenient)"""
        if relevance_score >= 60:  # Reduced from 70 to 60
            return "High"
        elif relevance_score >= 35:  # Reduced from 45 to 35
            return "Medium"
        else:
            return "Low"
    ...
    def generate_feedback(
        ...
        # Recommendations based on score (made very lenient)
        if relevance_score < 35:  # Reduced from 45 to 35
        ...
        elif relevance_score < 60:  # Reduced from 70 to 60
